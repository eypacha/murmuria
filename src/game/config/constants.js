export const DEBUG_MODE = false 
export const FOAM_ANIMATION = true
export const GRID_WIDTH = 50
export const GRID_HEIGHT = 50
export const TILE_SIZE = 64
export const DEPTH_GRID = 0
export const DEPTH_BUILDINGS = 10
export const DEPTH_RESOURCES = 20
export const DEPTH_UNITS = 30
export const UNIT_RENDER_OFFSET_Y = 48
export const CAMERA_DEFAULT_ZOOM = 1
export const CAMERA_MIN_ZOOM = 0.75
export const CAMERA_MAX_ZOOM = 1.75
export const CAMERA_PAN_SPEED = 650
export const CAMERA_WHEEL_ZOOM_RATE = 0.0012
export const SIMULATION_TICK_MS = 500
export const PAWN_ARRIVAL_THRESHOLD = 4
export const PAWN_PREPARE_TO_TREE_MS = 1000
export const PAWN_PREPARE_TO_GATHER_MS = 1000
export const PAWN_GATHER_DURATION_MS = 5000
export const PAWN_PREPARE_TO_RETURN_MS = 1000
export const PAWN_WOOD_HARVEST_CHUNK = 10
export const PAWN_GOLD_HARVEST_CHUNK = 10
export const PAWN_MEAT_HARVEST_CHUNK = 10
export const PAWN_CARRY_CAPACITY_WOOD = 5
export const PAWN_CARRY_CAPACITY_GOLD = 5
export const PAWN_CARRY_CAPACITY_MEAT = 20
export const TREE_WOOD_AMOUNT = 50
export const SHEEP_MEAT_AMOUNT = 20
export const INITIAL_GOLD_COUNT = 12
export const INITIAL_PAWNS = 2
export const INITIAL_TREE_COUNT = 30
export const INITIAL_SHEEP_COUNT = 8
